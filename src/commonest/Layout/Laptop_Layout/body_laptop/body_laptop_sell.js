...
function Body_laptop() {
  ...
  React.useEffect(() => {
    const API_phone = async () => {
      let laptop = await Apiproduct.getProducts({category: "LT"});
      while (laptop.length < 3) {
        laptop = laptop.concat(laptop);
      }
      setProduct_phone(laptop);
    };
              ...
              {product_phone &&
                ...
                product_phone.map((e, index) => (
                  <div className={Styles("smart_phone")} key={index}>
                    ...
                    <ul>
                      {e.specifications.split(",", 9).map((ee) => (
                        <li>{ee}</li>
                      ))}
                    </ul>
